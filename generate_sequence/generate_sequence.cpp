...
void GenerateInitialState()//初始化state
{
	...
	// generate the weight matrix  生成权重矩阵
	...
	//double temp;   表示当前t时刻所有概念节点的激活度，即输入在t时刻的状态
	for (int s = 0; s < Ns; s++)
	{
...
double TransFunc(double pop0)// sigmod函数
{
...
int main()
{
	...
	fp11 = fopen("TimeSequence.csv", "a+");
	fp2 = fopen("Network.txt", "a+");//建立一个只写文件
	fp22 = fopen("Network.csv", "a+");
